...
class TestWithFilterTests {
	...
	static class ContextConfiguration {
		...
		@Bean
		WebFilter testWebFilter() {
			return (exchange, chain) ->  Mono.just(exchange)
					.tap(contextView -> new DefaultSignalListener<>() {
						@Override
						public void doOnNext(ServerWebExchange webExchange) {
							System.out.println("just checking MDC: " + MDC.getCopyOfContextMap().toString());
						}
					})
